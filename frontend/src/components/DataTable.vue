<template>
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    ...
    <div class="overflow-x-auto">
      <table
        class="w-full text-sm text-left text-gray-500 dark:text-gray-400 min-w-full"
      >
        <thead
        ...
        >
          <tr>
            ...
            <th class="px-6 py-3 whitespace-nowrap" v-if="showActions">
              Ações
            </th>
          </tr>
        </thead>
    ...
    <!-- Paginação -->
    <div
      class="flex flex-col md:flex-row justify-between items-center px-4 py-3 bg-white dark:bg-gray-900 gap-4"
    >
      <Select
  ...
  const loadInfo = async () => {
    ...
    try {
      ...
      const response = await request.get(
        `${props.dataLink}?page=${currentPage.value}&itemsPerPage=${itemsPerPage.value}${filters}`
      )
